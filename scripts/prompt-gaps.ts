...
import { movementGapPrompt, muscleGapPrompt } from "../src/lib/gap-prompts";
...
// Movement prompt template lives in src/lib/gap-prompts.ts (shared with the
// /coverage UI). `promptForSparseMovement` delegates so there's one source.
function promptForSparseMovement(g: RegionMovementGap): string {
  return movementGapPrompt({ region: g.region, movement: g.movement });
}
...
// Muscle prompt template lives in src/lib/gap-prompts.ts (shared with the
// /coverage UI). `promptForMuscleCoverage` delegates so there's one source.
function promptForMuscleCoverage(group: MuscleGap[]): string {
  return muscleGapPrompt(group);
}
